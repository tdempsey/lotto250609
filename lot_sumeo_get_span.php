<?php
	...
	require ("includes/print_column_test_sumeo_span.php"); #200915
	...
	$sumeo = 5; ### <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
		...
		echo "<h2>*** SumEO = $sum,$even,$odd - sumeo $sumeo - Span $span</h2>";
		...
		*/
		...
		for ($col = 1; $col <= 5; $col++)
		{
			print_column_test_sumeo_span($col, $sumeo_sum, $sumeo_even, $sumeo_odd, $span); #220812
		}

<?php
	...
	### build lastxdraws
		...
		$query_dup = "SELECT * FROM combo_5_39_ends_dup_";
		$query_dup .= "$hml ";
		$query_dup .= "WHERE last_updated < '$curr_date' ";
		$query_dup .= "AND   wheel_percent_wa >=  0.2 ";
		$query_dup .= "ORDER BY b1, b2, b3 ASC ";
		...
		while($row = mysqli_fetch_array($mysqli_result_dup))
		{
			...
			$query9 = "UPDATE combo_5_39_ends_dup_";
			$query9 .= "$hml ";
	...
	#---------------------------------------------------------------------------------------------------------------
		...
		foreach ($sum_array as $sum_select)
		{
			...
			$sql = "SELECT * FROM ga_f5_dup_5_39_$hml WHERE `dup1` <= 1 AND `dup2` <= 2 AND `dup3` <= 2 AND `count` > 0 ORDER BY `count` DESC ";
			...
			while($row = mysqli_fetch_array($mysqli_result_dup))
			{
				echo "<p>dup - <strong>$row[dup1],$row[dup2],$row[dup3]</strong><br>";

				$sql5 = "SELECT * FROM `ga_f5_draw_count_5_39_100` WHERE `count` > 6 ORDER BY `count` DESC ";

				echo "$sql5<br>";

				$mysqli_result5 = mysqli_query($sql5, $mysqli_link) or die (mysqli_error($mysqli_link));

				while($row5 = mysqli_fetch_array($mysqli_result5))
				{
					echo "draw - <strong>$row5[draw0],$row5[draw1],$row5[draw2],$row5[draw3]</strong><br>";

					$sql7 = "SELECT * FROM `ga_f5_combo_count_5_39_$hml` WHERE count > 1 ORDER BY `count` DESC ";
					...
					{
						echo "combo - <strong>$row7[combo2],$row7[combo3],$row7[combo4],$row7[combo5]</strong><br>";
						...
						foreach ($col1_array as $col1_select)
						{
							...
							foreach ($col2_array as $col2_select)
							{
								...
								$query_dup .= "AND   rank0 <= $rank_limit[0] ";
								$query_dup .= "AND   rank1 <= $rank_limit[1] ";
								$query_dup .= "AND   rank2 <= $rank_limit[2] ";
								#$query_dup .= "AND   rank2 <= 2 ";
								$query_dup .= "AND   rank3 <= $rank_limit[3] ";
								$query_dup .= "AND   rank4 <= $rank_limit[4] ";
								$query_dup .= "AND   rank5 <= $rank_limit[5] ";
								$query_dup .= "AND   rank6 <= $rank_limit[6] ";

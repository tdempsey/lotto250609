<?php
	...
	print("<H2>Combo2 - position $combo_pos - $game_name - HML $hml</H2>\n");
	...
	if ($hml)
	{
		$query_all .= "WHERE draw_sum >= $range_low ";
		$query_all .= "AND   draw_sum <= $range_high ";
	}
	...
	for ($x = 1; $x <= $balls; $x++)
	{
		for ($y = $x + 1; $y <= $balls; $y++) ### $y = 1
		{
			if ($x <> $y)
			{
				...
				if ($hml)
				{
					$query5 .= "AND draw_sum >= $range_low ";
					$query5 .= "AND draw_sum <= $range_high ";
				}
				...
				if ($hml)
				{
					$query_last .= "AND draw_sum >= $range_low ";
					$query_last .= "AND draw_sum <= $range_high ";
				}

<?php
	...
	$filename = 'C:\Bitnami\wampstack-8.1.3-0\apache2\htdocs\lotto\mm210112.txt';
	echo "$filename<p>";
	$fp = fopen("$filename", "r");
	...
	for ($x = 0; $x <= ($size_date-2); $x++)
	{
		...
		#$mm_ball = explode(" ", $matches_mm[0][$x]);

		#print_r ($mm_ball);

		print "$draws[0]-$draws[1]-$draws[2]-$draws[3]-$draws[4] MM $mm_ball[1]<br> ";

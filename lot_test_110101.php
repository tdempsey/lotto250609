<?php
	...
	function print_sum_table($limit)
	{
		...
		for ($x = 40; $x < 240; $x++)
		{
			...
			$sum_temp_30 = number_format(($sum_count_array[$x][6]/365)*100,1);
			...
			$sum_temp_365 = number_format(($sum_count_array[$x][10]/(365*5))*100,1);
	...
	function print_wheel_sum_table($limit)
	{
		...
		$query_wheel = "SELECT * FROM $draw_prefix";
		$query_wheel .= "wheels_generated ";
		$query_wheel .= "ORDER BY sum, eo50 ASC ";
	
		$mysqli_result_wheel = mysqli_query($query_wheel, $mysqli_link) or die (mysqli_error($mysqli_link));

		$num_rows_all_wheel = mysqli_num_rows($mysqli_result_wheel); 
		...
		while($row_wheel = mysqli_fetch_array($mysqli_result_wheel))
		{
			...
			# convert eo50 into even,odd,d501,d502
			$query_eo50 = "SELECT * FROM $draw_prefix";
			$query_eo50 .= "eo50 ";
			$query_eo50 .= "WHERE id = $row_wheel[eo50] ";
			...
			$query_draw = "SELECT sum,date FROM $draw_table_name ";
			$query_draw .= "WHERE sum = $row_wheel[sum] ";
			...
			print("<TR>\n");
			print("<TD align=center><center>$row_wheel[sum]</center></TD>\n");
			print("<TD align=center><center>$row_wheel[eo50]</center></TD>\n");
			...
			print("<TD align=center><center>$row_wheel[num_rows]</center></TD>\n");
			for ($d = 0; $d <= 15; $d++) 
			{
			...
			for ($x = 40; $x < 240; $x++)
			{
				...
				if ($game == 6)
				{
					$weighted_average = (
						...
						($sum_table_500[$x]/500*100*0.20) +
						#($sum_table_1000[$x]/1000*100*0.05) +
						($sum_table_1000[$x]/$num_rows_all_draw*100*0.10));
				} else {
					$weighted_average = (
					($sum_table_10[$x]/10*100*0.05) +
					($sum_table_30[$x]/30*100*0.10) +
					...
					($sum_table_500[$x]/500*100*0.10) +
					($sum_table_1000[$x]/1000*100*0.20)) +
					#($sum_table_5000[$x]/$num_rows_all_draw*100*0.20));
					...
					($temp500/500*100*0.10) +
					($temp1000/1000*100*0.20));
					#($temp5000/($num_rows_all_draw-1)*100*0.20));
					...
					$x == 199 || $x == 209 || $x == 219 || $x == 229)
				{
					...
					#$sub_temp_5000 = number_format(($sub_sum[8]/$num_rows_all_draw)*100,1);
				...
				$query4 .= "'0', ";
				$query4 .= "'0', ";
				$query4 .= "'$row_last[last_date]',"; 
				$query4 .= "'$row_prev[last_date]')";
	...
	{
		global $debug, $draw_table_name, $balls, $balls_drawn, $game_includes, $draw_prefix, $game_includes; 
		...
		print("<TD BGCOLOR=\"#CCCCCC\"><font size=\"-1\">Triplet</font></TD>\n");
		...
		while($row = mysqli_fetch_array($mysqli_result))
		{
			...
			*/
			...
			for ($x = 0; $x < $balls_drawn; $x++)
			{
				if ($draw[$x] < ${col.$x.'_low'} || $draw[$x] > ${col.$x.'_high'}) {
					print("<TD BGCOLOR=\"#ff0000\"><CENTER><B><FONT COLOR=\"#ffffff\">$draw[$x]</FONT></B></CENTER></TD>\n");
				} else {
			...
			$table_wheel_percent = TestWheelTable($row[even],$row[odd],$row[d501],$row[d502],$row[sum],$table_wheel_total,$prev_wheel_percent);
			$table_wheel_total--;
			...
			$query_test = "SELECT * FROM $draw_prefix";
			$query_test .= "temp_125_nums ";
			$query_test .= "WHERE  d1 = $draw[0] ";
			$query_test .= "AND  d2 = $draw[1] ";
			$query_test .= "AND  d5 = $draw[4] ";

			//print "$query_test<p>";
			
			#$mysqli_result_test = mysqli_query($query_test, $mysqli_link) or die (mysqli_error($mysqli_link)); #fix 001

			#$num_rows = mysqli_num_rows($mysqli_result_test); #fix 001

			if ($num_rows) {
				$row_test = mysqli_fetch_array($mysqli_result_test);
				print("<TD><CENTER>{$row_test[count_12]}/{$row_test[count_15]}</CENTER></TD>\n");
			} else {
				print("<TD><CENTER>---</CENTER></TD>\n");
				$triplet_tot++;
			}
	...
	{
		global $debug,$min_test,$max_test,$date_test,$game,$draw_prefix,$game_includes;
	...
	*/
	...
	print "<a href=\"build_draw_sum.php\" target=\"_blank\">Open build_draw_sum.php</a>"; 

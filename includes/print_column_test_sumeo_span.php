<?php
	...
	// ----------------------------------------------------------------------------------
	function print_column_test_sumeo_span($col, $sum, $even, $odd, $span)
	{
		global $debug, $draw_table_name, $balls, $balls_drawn, $draw_prefix, $game, $hml, $span_low, $span_high,$col1_select; 
		...
		$temp_table = 'temp_column_sumeo_' . $sum . '_' . $even . '_' . $odd . '_' . $col. '_rng' . $span; 
		...
		echo "sum = $sum<br>";
		echo "span = $span<br>";
		echo "span_low = $span_low<br>";
		echo "span_high = $span_high<br>";

		$query = "SELECT * FROM $draw_table_name ";
		$query .= "WHERE date >= '2015-10-01' ";
		$query .= "AND (sum >= $span_low ";
		$query .= "AND sum <= $span_high) ";
		$query .= "AND even = $even ";
		$query .= "AND odd = $odd ";
		...
		while($row = mysqli_fetch_array($mysqli_result))
		{
			...
			if ($game == 10 or $game == 20)
			{
				...
				if ($hml)
				{
					$query2 .= "WHERE sum >= $span_low  ";
					$query2 .= "AND   sum <= $span_high  ";
					if ($col1_select)
					{
		...
		{	
			...
			if ($game != 10 OR $game != 20)
			{
				...
				if ($hml)
				{
					$query_date .= "AND sum >= $span_low  ";
					$query_date .= "AND   sum <= $span_high  ";
				}
			} else {
			if ($hml)
				{
					$query_date .= "WHERE sum >= $span_low  ";
					$query_date .= "AND   sum <= $span_high  ";
				}
			}

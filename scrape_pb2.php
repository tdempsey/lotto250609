<?php
	...
	$filename = 'C:\Bitnami\wampstack-8.1.3-0\apache2\htdocs\lotto\pb201130.txt';
	echo "$filename<p>";
	$fp = fopen("$filename", "r");
	...
	preg_match_all("(Megaball: \d{2})", $contents, $matches_pb);
	...
	for ($x = 0; $x <= ($size_date-2); $x++)
	{
		...
		#$pb_ball = explode(" ", $matches_pb[0][$x]);

		#print_r ($pb_ball);

		print "$draws[0]-$draws[1]-$draws[2]-$draws[3]-$draws[4] PB $pb_ball[1]<br> ";

<?php
	...
	$query4 .= "`num3` tinyint(3) unsigned NOT NULL,";
	$query4 .= "`num4` tinyint(3) unsigned NOT NULL,";
	...
	print("<H2>Combo4 - position $combo_pos - $game_name - Limit $limit</H2>\n");
	...
	print("<TD bgcolor=\"#CCCCCC\">Num4</TD>\n");
	...
	for ($w = 1; $w <= $balls; $w++)
	{
		for ($x = $w + 1; $x <= $balls; $x++) ### $x = 1
		{
			for ($y = $x + 1; $y <= $balls; $y++) ### $x = 1
			{
				for ($z = $y + 1; $z <= $balls; $z++) ### $x = 1
				{
					if ($w <> $x && $x <> $y && $y <> $z)
					{
						$sigma = 0;
						...
						$query5 .= "WHERE d1 = $w AND d2 = $x AND d3 = $y  AND d4  = $z and combo = $combo_pos "; 
						...
						$query_last .= "WHERE d1 = $w AND d2 = $x AND d3 = $y  AND d4  = $z and combo = $combo_pos ";  

<?php
	...
	require ("includes/dup_table_update.php");
	...
	function calculate_wheel_count ($col1,$sum,$row)
	{
		...
		// 180
		$month6_Ymd = gmdate ('Y-m-d', $month6_unix);
		$query3 = "SELECT * FROM $draw_table_name ";
		$query3 .= "WHERE sum	= $sum ";
		$query3 .= "AND	b1	= $col1 ";
		$query3 .= "AND	even	= $row[even] ";
	...
	{
		global $debug, $draw_table_name, $balls, $balls_drawn, $game_includes, $draw_prefix, $game_includes, $hml, $range_low, $range_high, $col1_select, $balls, $balls_draws; 
		...
		print("<TD BGCOLOR=\"#CCAACC\">DC0</TD>\n");
		...
		while($row = mysqli_fetch_array($mysqli_result))
		{
			...
			print("<TD nowrap><CENTER>$row[0]</CENTER></TD>\n");
			...
			if ($row[6] >= 70 AND $row[6] <= 140) ### 201223
			{
				$query7 = "SELECT * FROM combo_5_";
				$query7 .= "$balls";
				$query7 .= "_";
				$query7 .= "$balls_draws";
				$query7 .= "_";
			...
			$x10_array = Last10Count($row[0]); ### 201223
			...
			#$query_seq = "SELECT * FROM $draw_prefix";	### 230+917
			#$query_seq .= "seq_table ";
			...
			$date_nodash = str_replace("-","", $row[0]); ### 201223
			...
			for ($k = 1; $k <= 35; $k++)
			{
				...
				for ($m = $k; $m <= $k+4; $m++)
				{
					if ($row[1] == $m || $row[2] == $m || $row[3] == $m || $row[4] == $m || $row[5] == $m) ### 201223
					{
						$kounter++;
					}
				}
		...
		#$num_temp = number_format($hot_sum/($limit*6),2)*100;	###230917
		print("<TD><CENTER>$num_temp%</CENTER></TD>\n");
		#$num_temp = number_format($warm_sum/($limit*6),2)*100;	###230917
		print("<TD><CENTER>$num_temp%</CENTER></TD>\n");
	...
	function print_dup_table($limit,$min_max)
	{
		...
		while($row = mysqli_fetch_array($mysqli_result))
		{
			...
			if ($min_max == 0)
			{
				...
				$date_test[$z] = $row[0]; ### 201223
			}

			print("</TR>\n");
			$z++;
			$last_date = $row[0]; ### 201223
		}
	...
	function print_column_test($col)
	{
		...
		#echo "### enter print_column_test ###<br>";
		...
		while($row = mysqli_fetch_array($mysqli_result))
		{
			...
			#$draw_date_array = date_parse("$row[0]"); ### 210104
		...
		{	
			...
			{
				print("<TD align=center>---</TD>\n");
			} elseif ((strtotime ("$prev_date") - $month6_unix) < 0) {
			...
			$sum_temp_y1 = number_format(($column_count_array[$x][6]/365)*10,1);
			...
			$sum_temp_y5 = number_format(($column_count_array[$x][10]/(365*5))*10,1);
			...
			$sum_temp = number_format(($weighted_average*0.01),2);
		...
		### start sorted table ####################################################################
		print("<h3>Column $col Test - Sort by percent_wa</h3>\n");
		print("<TABLE BORDER=\"1\">\n");
		...
		{
			print("<TD align=center>$row3[0]</TD>\n");
		...
		#echo "### exit print_column_test ###<br>";
	...
	function print_column_test_col_x_y($col1,$col2)
	{
		...
		for ($col = $col1; $col <= $col2; $col++)
		{
			...
			while($row = mysqli_fetch_array($mysqli_result))
			{
				...
				#$draw_date_array = date_parse("$row[0]");
				$draw_date_array = explode("-","$row[0]"); ### 210104
	...
	#echo "### test_combin_print_all ###<br>";

	#$last_date = test_combin_print_all(100); 

	#test_combin_print_all_detail(10); #200123

	#$last_date = test_combin_print_hml(100); 

	#$last_date = test_combin_print_sum(100);	###221209

<?php
	// ----------------------------------------------------------------------------------
	function split_sumeo_5_no_tables($sum,$even,$odd)
	{
		...
		*/
		...
		echo "1 - no filter - num_rows copied = $cnt[0]<p>";

		##################### LIMIT ###################################################################

		$limit_temp = array_fill(0,2,0);
		$limit_array = array_fill(0,10,$limit_temp);
		...
		echo "filter by range - $query_copy<br>";
		...
		*/
		...
		$table_temp2 = 'temp2_' . $balls_drawn . '_' . $balls . '_' . $sum . '_' . $even . '_' . $odd;
		...
		while ($row_drange4 = mysqli_fetch_array($mysqli_result_drange4))
		{
			...
			echo "3 - num_rows_sumeo5 = $num_rows_sumeo5<p>";

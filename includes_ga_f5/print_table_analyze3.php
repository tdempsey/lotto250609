<?php
	// ----------------------------------------------------------------------------------
	function print_table3($combin,$limit)
	{
		...
		while($row = mysqli_fetch_array($mysqli_result))
		{
			...
			if ($row[8] == 0 && $row[9] == 0)
			{
				require ("includes/update_even_odd3.incl");
			...
			if ($even <= 0 || $even >= 4) 
			{
				print("<TD BGCOLOR=\"#99CCFF\"><strong><CENTER>$row[8]/$row[9]</CENTER></strong></TD>\n");
			} else {
				print("<TD BGCOLOR=\"#99CCFF\"><CENTER>$row[8]/$row[9]</CENTER></TD>\n");
			}
			...
			$query3 .= "AND   even = $row[8] ";
			$query3 .= "AND   odd = $row[9] ";
